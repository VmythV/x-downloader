...
客户端不能提交任意输出目录、文件名、FFmpeg 参数或非候选媒体 URL。
...
`/v1/status` 返回 `apiVersion`、Helper 版本、FFmpeg 状态、下载目录可写性、代理、并发数、持久化能力、候选数量和任务统计。扩展要求匹配的 API 版本，所有 Helper 请求有 20 秒超时。
...
候选和任务使用 0600 权限的原子 JSON 文件保存。Helper 保留最近 300 个候选和 500 个任务。重启时残留的 `queued/downloading` 会转换为 `failed` 并标记为被重启中断，允许用户重试。
...
文件名模板支持 `{date}`、`{author}`、`{postId}`、`{mediaIndex}`、`{mediaId}`、`{width}`、`{height}` 和 `{ext}`。路径分隔符被拒绝，DOM 元数据会清理并限制长度。
...
- 暂无签名安装器、自动启动、自动更新和 Helper 配置图形界面。

## 11. 验证

- JavaScript 覆盖 master 解析、质量选择、重写、Fetch 观察、设置和帖内状态。
- Go 覆盖 URL 限制、音轨配对、API、原子状态文件、候选/任务恢复和下载命名。
