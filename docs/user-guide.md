...
目录保存在 Helper 的 `stateDir/x-downloader.sqlite3`，关闭浏览器或重启 Helper 后仍会使用相同位置。修改目录不会搬移已完成文件，也不会改变已经排队或下载中的任务。
...
- 等待和下载任务会显示进度条。能计算总时长时显示百分比；不能计算时显示动态进度条、已处理时长和速度。

Helper 会把完整历史保存在 SQLite 中，不再把数据库限制为 300 个候选或 500 个任务。运行时只缓存最近 300 个候选和 500 个已结束任务，扩展单个标签页也会限制缓存量，避免长时间浏览信息流后持续占用内存；这不会删除数据库历史。

### 网页管理台

Helper 运行时打开 `http://127.0.0.1:17890/`，或点击扩展弹窗右上角的 ↗：

1. 输入与扩展相同的配对令牌并连接。令牌只进入当前标签页的 `sessionStorage`，关闭标签页后清除。
2. “概览”查看总任务数、成功率、下载体积、近 30 天趋势、错误、作者、标签和分辨率统计。
3. “任务”按状态、文件名或媒体 ID 筛选并分页加载，也可取消、重试或显示文件。
4. “历史”搜索作者、帖子 ID、备注或文件名，并按状态、标签过滤；可以添加备注、标签或删除本地历史记录。
5. “设置”修改下载目录、命名模板、并发和重试；“标签”可创建、编辑和删除标签。
6. 顶部“刷新”用于手动刷新；自动刷新可以关闭或设置为 2、5、10、30 秒。自动刷新只更新运行状态与统计，不会覆盖正在编辑的配置或折叠已经加载的分页结果。

历史搜索对三个及以上字符使用 SQLite FTS5 索引，短关键词使用转义后的模糊匹配。任务和历史都采用游标分页，数据增长到上万条时不会一次加载到浏览器。
...
- 已完成历史仍可查看和定位。
- 等待中的任务会继续排队。
- 重启时正在下载的任务，如果还有剩余尝试次数，会清理临时文件后重新排队；没有剩余次数时标记为失败。
- 已经生成的 MP4 不会被删除。
- 设置页保存的下载目录会自动恢复。
...
先退出 Helper，再删除配置中的 `stateDir`。这会清理 SQLite 数据库、候选、任务、备注、标签和运行时设置，不删除已经完成的 MP4。只想删除单条记录时，可以在网页管理台的“历史”页操作。

### 从旧版升级

首次启动新版 Helper 时，会把 `stateDir` 中原有的 `candidates.json`、`jobs.json` 和 `settings.json` 一次性导入 SQLite。导入在事务中完成，通过完整性检查后原文件改名为 `.migrated-v1.bak`。确认新版历史和设置正确后，可以自行删除这些备份。
