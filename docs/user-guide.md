...
下载目录可以位于外置磁盘或其他挂载卷。临时目录和下载目录不在同一个文件系统时，Helper 会先复制到目标目录中的隐藏暂存文件，完整落盘后再改为最终文件名，不会把半成品暴露为完成文件。
...
- 若旧版本曾提示 `cross-device link`，请更新并重启 Helper，然后重新下载；旧版本已经清理的临时文件无法直接恢复。
