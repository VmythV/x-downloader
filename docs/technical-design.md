...
X 播放器请求 master
  ...
  → service worker 携带 bearer token 注册候选
...
内容页面不能为单个下载任务提交输出目录、文件名、FFmpeg 参数或非候选媒体 URL。只有通过配对认证的扩展设置页可以更新全局下载目录，Helper 会要求绝对路径并验证目录可写。
...
`/v1/status` 返回 `apiVersion`、Helper 版本、FFmpeg 状态、下载目录可写性、代理、并发数、持久化能力、候选数量和任务统计。当前 API 版本为 2；扩展要求精确匹配。普通 Helper 请求有 20 秒超时，等待用户操作的文件夹选择请求允许 5 分钟。

设置页不依赖浏览器暴露本地绝对路径，而是让 Helper 调用操作系统目录选择器。macOS 使用 AppleScript，Windows 使用 `FolderBrowserDialog`，Linux 依次尝试 `zenity` 和 `kdialog`。选中的目录经用户确认保存后写入 `stateDir/settings.json`，并立即应用于之后创建的任务。

## 7. 下载队列与持久化

并发数允许 1–4，队列容量为 100。相同候选与 variant 的活动任务会去重；成功文件仍存在时重复提交直接返回已完成任务。
...
文件名模板支持 `{date}`、`{author}`、`{postId}`、`{mediaIndex}`、`{mediaId}`、`{width}`、`{height}` 和 `{ext}`。路径分隔符被拒绝，DOM 元数据会清理并限制长度。配置中的 `downloadDir` 是设置页的默认值；设置页保存过目录后，持久值优先，直到用户恢复默认。
...
- 暂无签名安装器、自动启动和自动更新；FFmpeg 路径、并发数和命名模板仍需通过配置文件调整。
